...
bool isScrolling = false; // 轮播文字动画是否进行中
int scrollAnimPos = 0; // 轮播文字动画当前位置
String scrollAnimText = ""; // 当前轮播的文字
...
Task tScrollText(5000, TASK_FOREVER, &tScrollTextCallback, &runner, true); // 5秒触发一次轮播
Task tScrollAnim(30, TASK_FOREVER, &tScrollAnimCallback, &runner, false); // 30毫秒滚动一帧文字
...
Task tGetLocalTemp(30*1000,TASK_FOREVER,&tGetLocalTempCallback,&runner,true); //每隔30秒读一次本地温湿度
// 启动runner
void startRunner(){
  runner.startNow();
}
...
// 轮播ScrollText任务（触发器，每5秒启动一次动画）
void tScrollTextCallback(){
  if(!isScrolling && scrollText[currentIndex]){
    isScrolling = true;
    scrollAnimPos = 24;
    scrollAnimText = scrollText[currentIndex];
    tScrollAnim.enable();
  }
}
...
void tScrollAnimCallback(){
  ...
  clk.fillSprite(backFillColor);
  clk.setTextDatum(CC_DATUM);
  clk.drawString(scrollAnimText, 80, scrollAnimPos + 12);
